...
def get_ni_data(meta_file_path: str) -> dict:
    bin_file_path = get_data_meta_path(meta_file_path)[0]
    ni_data = {'meta': None, 't_ms': None, 'adc': None, 'din': None}
    ...
    try:
        ni_data['din'] = parse_digital(data_mmap_nidq, dig_words=n_dig_word)
    except NotImplementedError:
        warnings.warn('Cant decode {} words of digital data yet'.format(n_dig_word))
    return ni_data
...
def quick_frames(data: np.array, stamps, samp_pre: int=-300, samp_post: int=3000) -> np.ndarray:
    ...
    all_frames_idx = np.repeat(np.arange(samp_window), n_trigs).reshape(-1, n_trigs) 
    all_frames_idx = all_frames_idx + samp_pre + stamps
    flat_frames = data[:, all_frames_idx.T.flatten()]
    frames_3d = flat_frames.reshape(-1, n_trigs, samp_post - samp_pre, order='c') # [n_ch, n_trig, n_sampl_window]
    return frames_3d